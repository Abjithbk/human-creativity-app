           ...
           image ? (
            <div className="w-full h-full rounded-full border-2 border-transparent overflow-hidden">
                {isVideo ? (
                   ...
                   <img src={image} alt="My Story" className="w-full h-full object-cover" />
                )}
            </div>
           ) : (
...
export default function Dashboard() {
  ...
  const groupedStories = stories.reduce((acc: any, story) => {
    const ownerName = story.owner?.username;
    if (!ownerName) return acc;
    if (!acc[ownerName]) {
      acc[ownerName] = {
        username: ownerName,
    ...
    acc[ownerName].items.push(story);
    return acc;
  }, {});

  const allGroups = Object.values(groupedStories) as any[];

  const myStoriesGroup = allGroups.find((group) => group.owner_id === currentUserId);
  const otherStoriesGroups = allGroups.filter((group) => group.owner_id !== currentUserId);
  ...
  const handleGroupComplete = () => {
    ...
    const playlist = [myStoriesGroup, ...otherStoriesGroups].filter(Boolean);
    const currentGroupOwnerId = activeStoryGroup[0].owner_id;
    const currentIndex = playlist.findIndex((g: any) => g.owner_id === currentGroupOwnerId);
    if (currentIndex !== -1 && currentIndex < playlist.length - 1) {
      const nextGroup = playlist[currentIndex + 1];
      openStoryViewer(nextGroup.items, nextGroup.owner_id);
    } else {
      setActiveStoryGroup(null);
    }
  };

  const handleLocalDelete = (deletedStoryId: number) => {
    setStories((prevStories) => prevStories.filter(s => s.id !== deletedStoryId));

    if (activeStoryGroup) {
      const updatedGroup = activeStoryGroup.filter(s => s.id !== deletedStoryId);
  ...
  const handleLocalPostDelete = (deletedPostId: number) => {
    setPosts((prevPosts) => prevPosts.filter(p => p.id !== deletedPostId));
  };

  const openStoryViewer = (storiesArray: any[], ownerId: number) => {
    setActiveStoryGroup(storiesArray); 

    if (!viewedUsers.includes(ownerId)) {
      const newViewed = [...viewedUsers, ownerId];
    ...
    <div className="min-h-screen bg-white dark:bg-black text-zinc-900 dark:text-white font-sans flex overflow-hidden selection:bg-purple-500 selection:text-white transition-colors duration-300 ">
      ...
      <aside className="w-64 flex flex-col justify-between p-6 border-r border-zinc-200 dark:border-zinc-900 h-screen sticky top-0 transition-colors duration-300">
        <div>
          <div onClick={() => setActiveTab('Home')} className="flex items-center gap-3 mb-10 cursor-pointer">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-cyan-400 to-purple-600 flex items-center justify-center"><div className="w-3 h-3 bg-white rounded-full"></div></div>
            <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500">Canvas</h1>
          </div>
        ...
        <div className="flex flex-col items-center gap-4">
          <Link href='/create' className='w-full'><button className="flex-1 w-full py-3 rounded-full bg-gradient-to-r from-purple-600 to-cyan-500 text-white font-semibold shadow-lg shadow-purple-500/20 flex items-center justify-center gap-2 hover:opacity-90 transition transform active:scale-95"><Plus size={20} /> Create</button></Link>
          <button onClick={() => setDarkMode(!darkMode)} className="w-12 h-12 flex items-center justify-center rounded-full bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:text-purple-500 dark:hover:text-purple-400 transition-colors border border-zinc-200 dark:border-zinc-800">{darkMode ? <Sun size={20} /> : <Moon size={20} />}</button>
        </div>
      </aside>
           ...
           <div className="flex gap-6 mb-8 overflow-x-auto pb-4 no-scrollbar">
             ...
             {myStoriesGroup ? (
                 <StoryItem 
                    ...
                    name="Your Story" 
                    hasBorder 
                    isViewed={viewedUsers.includes(myStoriesGroup.owner_id)}
                    image={myStoriesGroup.items[myStoriesGroup.items.length-1].media_url} 
                    onClick={() => openStoryViewer(myStoriesGroup.items, myStoriesGroup.owner_id)}
                 />
             ) : (
                 <Link href="/create">
             ...
             {otherStoriesGroups.map((group: any) => (
                 <StoryItem 
                   key={group.username} 
                   name={group.username} 
                   hasBorder 
                   image={group.items[group.items.length - 1].media_url} 
                   isViewed={viewedUsers.includes(group.owner_id)}
                   onClick={() => openStoryViewer(group.items, group.owner_id)}
                 />
             ))}
             ...
             {posts.length === 0 ? (
               ...
               posts.map((post) => (
                 <PostCard 
                   ...
                   onDeletePost={handleLocalPostDelete}
                 />
               ))
             )}
           </div>
      ...
      <aside className="w-80 p-8 border-l border-zinc-200 dark:border-zinc-900 hidden xl:block h-screen sticky top-0 transition-colors duration-300">
        <div className="relative mb-8 group">
          ...
          <input type="text" placeholder="Search..." className="w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-full py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500/50 text-zinc-800 dark:text-zinc-300 transition-all placeholder:text-zinc-500 dark:placeholder:text-zinc-600"/>
        </div>
        <div className="bg-zinc-100 dark:bg-zinc-900/40 rounded-2xl p-5 mb-8 border border-purple-500/20 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-2 mb-2 text-purple-400"><ShieldCheck size={18} /><span className="font-semibold text-sm">Plagiarism Shield Active</span></div>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">We use perceptual hashing to protect your uploads. Your art belongs to you.</p>
        </div>
        <div>
          <h3 className="font-semibold mb-4 text-zinc-800 dark:text-zinc-200">Trending Humans</h3>
          <div className="space-y-5"><TrendingUser name="Artist Name" tag="Traditional Ink" /><TrendingUser name="Design Daily" tag="Digital Art" /><TrendingUser name="Pixel Jim" tag="Voxel Art" /></div>
        </div>
      </aside>

      {activeStoryGroup && (
        <StoryViewer 
          key={activeStoryGroup[0].owner_id}
          stories={activeStoryGroup} 
          onClose={() => setActiveStoryGroup(null)} 
          onGroupComplete={handleGroupComplete}
          currentUserId={currentUserId}
          onDeleteStory={handleLocalDelete}
        />
      )}
